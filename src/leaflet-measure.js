...
L.Control.Measure = L.Control.extend({
    ...
    options: {
        ...
        activeColor: '#FC9810', // base color for map features while actively measuring
        completedColor: '#C7013A', // base color for permenant features generated from completed measure
    ...
    _initLayout: function() {
        ...
        const $startLinear = $('.js-start-linear', container); // start linear measure button
        const $startArea = $('.js-start-area', container); // start area measure button
        ...
        L.DomEvent.on($startLinear, 'click', L.DomEvent.stop);
        L.DomEvent.on($startLinear, 'click', this._startMeasureLinear, this);
        L.DomEvent.on($startArea, 'click', L.DomEvent.stop);
        L.DomEvent.on($startArea, 'click', this._startMeasureArea, this);
    ...
    // expand the menu icon -> menu tab
    _expand: function() {
        dom.hide(this.$toggle);
        dom.show(this.$interaction);
    },

    // collapse the menu tab -> menu icon unless it is locked down (e.g. when measurement has started)
    _collapse: function() {
        if (!this._locked) {
    ...
    // 2. measure started/in progress but no points added
    ...
    // 3. measure started/in progress with points added *** //
    ...
    // set measureMode to linear measure
    _startMeasureLinear: function(){
        this._measureMode = 'linear-measure';
        this._startMeasure();
    },

    // set measureMode to area measure
    _startMeasureArea: function(){
        this._measureMode = 'area-measure';
        this._startMeasure();
    },
    ...
    _startMeasure: function() {
        ...
        this._map
            ...
            .on('keypress', this._handleKeyPress, this); // undo option
    ...
    _finishMeasure: function() {
        ...
        this._map
            ...
            .off('keypress', this._handleKeyPress, this); // undo option
    ...
    _handleMeasureDoubleClick: function() {
        ...
        if (latlngs.length === 1) {
            ...
            if (this._measureMode === 'linear-measure') {
                resultFeature = L.polyline(latlngs, this._symbols.getSymbol('resultLine'));
                popupContent = linePopupTemplateCompiled({
                    model: L.extend({}, calced, this._getMeasurementDisplayStrings(calced))
                });
    ...
    _handleMeasureClick: function(evt) {
        ...
        if (!lastClick || !latlng.equals(lastClick)) {
            ...
            if (this._measureMode === 'area-measure') {
    ...
    // handle key presses during ongoing measure
    // remove last clicked point, update measure layers and results ui
    _handleKeyPress: function(evt) {
        if (evt.originalEvent.key === 'Escape') {
            this._finishMeasure();
        } else if (this._latlngs && (evt.originalEvent.key === 'Delete' || evt.originalEvent.key === 'Backspace')) {  
            const vertexMarkers = this._measureVertexes.getLayers(),
                len = vertexMarkers.length,
                lastMarker = vertexMarkers[len - 1],
                prevMarker = vertexMarkers[len - 2],
                activeStyle = this._symbols.getSymbol('measureVertexActive');
            
            this._measureVertexes.removeLayer(lastMarker); // remove last vertex marker
            this._latlngs = this._latlngs.slice(0, -1); // remove last vertex coordinates
            this._addMeasureBoundary(this._latlngs); // update Perimeter

            if (this._measureMode === 'area-measure') {
                this._addMeasureArea(this._latlngs); // update Area
            }
            
            if (len > 1) {
                // reset previous vertex to an active class
                prevMarker.setStyle(activeStyle);
                
                if (prevMarker._path) { // set attribute on path directly
                    prevMarker._path.setAttribute('class', activeStyle.className); 
                }
    ...
    // re-compute measurement boundary
